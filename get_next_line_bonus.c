...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/04 10:45:40 by cboutier          #+#    #+#             */
/*   Updated: 2021/06/04 11:32:17 by cboutier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || !line || !str[fd])
		return (-1);
	index = ft_strchr(str[fd], '\n');
	if (index > 0)
	{
		tmp = str[fd];
		*line = ft_substr(str[fd], 0, index - str[fd]);
		str[fd] = ft_substr(str[fd], (index - str[fd]) + 1, ft_strlen(str[fd]));
		free(tmp);
		return (1);
	}
	else
